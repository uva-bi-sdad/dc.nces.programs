# Community Colleges Programs -- Census block group level FCAs
# packages
library(tigris)
library(sf)
library(dplyr)
...
inds_engin_rel <- st_within(geopts_engin_rel$geometry, blocks$geometry, sparse=T)
inds_engin <- st_within(geopts_engin$geometry, blocks$geometry, sparse=T)
inds_comp <- st_within(geopts_comp$geometry, blocks$geometry, sparse=T)
...
for (i in inds_engin_rel){
  if (identical(blocks$NAME[i],character(0))){
    ...
    counties_list_engin_rel<- append(counties_list_engin_rel, blocks$GEOID[i])}
}
...
for (i in inds_engin){
  if (identical(blocks$NAME[i],character(0))){
    ...
    counties_list_engin<- append(counties_list_engin, blocks$GEOID[i])}
}
...
for (i in inds_comp){
  if (identical(blocks$NAME[i],character(0))){
    ...
    counties_list_comp<- append(counties_list_comp, blocks$GEOID[i])}
}
...
out_df_engin <- left_join(out_df_engin, block_names, by=c("geoid"))
out_df_engin_rel <- left_join(out_df_engin_rel, block_names, by=c("geoid"))
out_df_comp <- left_join(out_df_comp, block_names, by=c("geoid"))
...
dbWriteTable(con, c("dc_education_training", "va_bg_osrm_2019_drive_times_nearest_engineering_program"),
             df_long_engin,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_bg_osrm_2019_drive_times_nearest_engineering_related_program"),
             df_long_engin_rel,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_bg_osrm_2019_drive_times_nearest_computer_science_program"),
             df_long_comp,  row.names = F)

# dbRemoveTable(con, c("dc_education_training", "va_bg_osrm_2019_drive_times_nearest_engineering_program"))
# dbRemoveTable(con, c("dc_education_training", "va_bg_osrm_2019_drive_times_nearest_engineering_related_program"))
# dbRemoveTable(con, c("dc_education_training", "va_bg_osrm_2019_drive_times_nearest_computer_science_program"))

dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_osrm_2019_drive_times_nearest_engineering_program
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_osrm_2019_drive_times_nearest_engineering_related_program
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_osrm_2019_drive_times_nearest_computer_science_program
                    OWNER TO data_commons")
...
# add region names
out_df_engin <- left_join(df3_engin, block_names, by=c("GEOID"="geoid"))
out_df_engin_rel <- left_join(df3_engin, block_names, by=c("GEOID"="geoid"))
out_df_comp <- left_join(df3_comp, block_names, by=c("GEOID"="geoid"))
...
dbWriteTable(con, c("dc_education_training", "va_bg_nces_2020_community_college_engineering_related_fca"),
             out_df_long_engin_rel,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_bg_nces_2020_community_college_engineering_fca"),
             out_df_long_engin,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_bg_nces_2020_community_college_computer_sciences_fca"),
             out_df_long_comp,  row.names = F)

# dbRemoveTable(con, c("dc_education_training", "va_bg_nces_2020_community_college_engineering_related_fca"))
# dbRemoveTable(con, c("dc_education_training", "va_bg_nces_2020_community_college_engineering_fca"))
# dbRemoveTable(con, c("dc_education_training", "va_bg_nces_2020_community_college_computer_sciences_fca"))

dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_nces_2020_community_college_engineering_related_fca
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_nces_2020_community_college_engineering_fca
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_bg_nces_2020_community_college_computer_sciences_fca
                    OWNER TO data_commons")
