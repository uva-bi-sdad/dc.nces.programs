# Community Colleges Programs -- Census tract level FCAs
# packages
library(tigris)
library(sf)
library(dplyr)
...
for (i in inds_engin_rel){
  if (identical(tracts$NAME[i],character(0))){
    ...
    counties_list_engin_rel<- append(counties_list_engin_rel, tracts$GEOID[i])}
}
...
for (i in inds_engin){
  if (identical(tracts$NAME[i],character(0))){
    ...
    counties_list_engin<- append(counties_list_engin, tracts$GEOID[i])}
}
...
for (i in inds_comp){
  if (identical(tracts$NAME[i],character(0))){
    ...
    counties_list_comp<- append(counties_list_comp, tracts$GEOID[i])}
}
...
out_df_engin <- left_join(out_df_engin, tract_names, by=c("geoid"))
out_df_engin_rel <- left_join(out_df_engin_rel, tract_names, by=c("geoid"))
out_df_comp <- left_join(out_df_comp, tract_names, by=c("geoid"))
...
dbWriteTable(con, c("dc_education_training", "va_tr_osrm_2019_drive_times_nearest_engineering_program"),
             df_long_engin,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_tr_osrm_2019_drive_times_nearest_engineering_related_program"),
             df_long_engin_rel,  row.names = F)
dbWriteTable(con, c("dc_education_training", "va_tr_osrm_2019_drive_times_nearest_computer_science_program"),
             df_long_comp,  row.names = F)

# dbRemoveTable(con, c("dc_education_training", "va_tr_osrm_2019_drive_times_nearest_engineering_program"))
# dbRemoveTable(con, c("dc_education_training", "va_tr_osrm_2019_drive_times_nearest_engineering_related_program"))
# dbRemoveTable(con, c("dc_education_training", "va_tr_osrm_2019_drive_times_nearest_computer_science_program"))

dbSendStatement(con, "ALTER TABLE dc_education_training.va_tr_osrm_2019_drive_times_nearest_engineering_program
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_tr_osrm_2019_drive_times_nearest_engineering_related_program
                    OWNER TO data_commons")
dbSendStatement(con, "ALTER TABLE dc_education_training.va_tr_osrm_2019_drive_times_nearest_computer_science_program
                    OWNER TO data_commons")
...
# add region names
out_df_engin <- left_join(df3_engin, tract_names, by=c("GEOID"="geoid"))
out_df_engin_rel <- left_join(df3_engin, tract_names, by=c("GEOID"="geoid"))
out_df_comp <- left_join(df3_comp, tract_names, by=c("GEOID"="geoid"))
